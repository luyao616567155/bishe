...
flags = tf.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'artificial1', 'Dataset string.')#第一个是参数名称，第二个参数是默认值，第三个是参数描述
...
support = preprocess_adj(adj,FLAGS.normalize)#support是邻接矩阵的归一化形式
...
placeholders = {
    ...
    'Xs': tf.placeholder(tf.float32, shape=(None,adj.shape[0],1)),#观测值**.x
    'y': tf.placeholder(tf.float32, shape=(None,adj.shape[0])),#预测值**.y
