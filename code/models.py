...
class Model(object):
    ...
    def build(self):
        ...
        # Build sequential layer model, make the output prediction建立序列层模型，进行输出预测
        ...
        var_list1 = [var for var in tf.trainable_variables() if not 'graph_' in var.name]#tf.trainable_variables()仅可以查看可训练的变量
        var_list2 = [var for var in tf.trainable_variables() if 'graph_' in var.name]
        opt1 = tf.train.AdamOptimizer(learning_rate=self.FLAGS.learning_rate)#tf.train.AdamOptimizer()函数是Adam优化算法：是一个寻找全局最优点的优化算法，引入了二次方梯度校正
        opt2 = tf.train.AdamOptimizer(learning_rate=self.FLAGS.graph_learning_rate)
        grads = tf.gradients(self.loss, var_list1 + var_list2)#tf.gradients()实现ys对xs求导
        grads1 = [tf.clip_by_norm(grad, self.FLAGS.max_grad_norm) for grad in grads[:len(var_list1)]]#clip by norm梯度裁剪的最直接目的就是防止梯度爆炸，手段就是控制梯度的最大范式。
        grads2 = [tf.clip_by_norm(grad, self.FLAGS.max_grad_norm) for grad in grads[len(var_list1):]]
        train_op1 = opt1.apply_gradients(zip(grads1, var_list1))#应用新gradients
...
class CoupledGNN(Model):
    def __init__(self, FLAGS,init_values,placeholders,node_input_features, n_nodes, **kwargs):
        ...
        #initialize self activation parameters初始化自激活参数
        with tf.variable_scope(self.name):#用于定义创建变量(层)的ops的上下文管理器
        ...
        #get input influence of each user得到每个用户的输入影响力
        ...
        self.inputs_influence = tf.contrib.layers.instance_norm(tf.transpose(tf.tile(
                                        ...
                                        ,data_format="NHWC")
        #get input state of each user得到每个用户的输入状态
    ...
    def _loss(self):
        # regularization of l2  l2正则化
        for var in tf.trainable_variables():
            self.loss += self.FLAGS.reg_l2 * tf.nn.l2_loss(var)


        #mean relative square loss平均相对平方损失
        ...
        #regularization of cross entropy交叉熵的正则化
        ...
        #the total loss to be optimized待优化总损失
